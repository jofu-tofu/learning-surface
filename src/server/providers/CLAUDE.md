...
- All system prompts are defined in `server/system-prompt.ts` — `TEACHING_PRINCIPLES` (shared pedagogy), `SYSTEM_PROMPT` (API mode), `CLI_SYSTEM_PROMPT` (CLI mode). If prompt content changes, update that one file.
