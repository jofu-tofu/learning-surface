...
export function parseFlowchartData(content: string): FlowchartData | null {
  try {
...
interface PositionedEdge extends FlowchartEdge {
  path: string;
  labelX: number;
  labelY: number;
}

export function computeFlowchartLayout(data: FlowchartData): {
...
export function FlowchartRenderer({ content }: RendererProps): React.ReactElement {
  const rawId = useId();
  const markerId = rawId.replace(/:/g, '');

  const data = useMemo(() => parseFlowchartData(content), [content]);
  const layout = useMemo(() => data ? computeFlowchartLayout(data) : null, [data]);

  if (!data || !layout) {
    return <ErrorBanner message="Invalid flowchart data — expected JSON with nodes and edges arrays" />;
  }

  if (layout.nodes.length === 0) {
    return <div className="text-sm text-surface-400">Empty flowchart</div>;
  }

  return (
    <div data-testid="canvas-flowchart" className="canvas-container flex justify-center">
      <svg
        ...
        {layout.edges.map((e, i) => (
          <g key={`edge-${i}`}>
            <path
              ...
              strokeOpacity={0.5}
              strokeWidth={1.5}
              markerEnd={`url(#fc-arrow-${markerId})`}
            />
            {e.label && (
              <text
                x={e.labelX}
