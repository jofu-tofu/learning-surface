  ...
  const renderCrumb = (v: VersionMeta, idx: number, isCurrent: boolean, faded: boolean) => {
    ...
    return (
      <React.Fragment key={v.version}>
        {(idx > 0 || faded) && (
          <span className="shrink-0 text-surface-600 text-xs select-none">›</span>
        )}
        <button
          data-testid={`crumb-${v.version}`}
          onClick={() => onVersionSelect?.(v.version)}
          className={`
            shrink-0 flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs transition-colors cursor-pointer border
            ${isCurrent
        ...
        >
          ...
          {branches > 1 && (
            <span
              ...
              className="shrink-0 ml-0.5 inline-flex items-center justify-center w-4 h-4 rounded-full text-[10px] font-medium bg-purple-500/20 border border-purple-500/30 text-purple-400 cursor-pointer hover:bg-purple-500/30"
            >
              {branches}
            </span>
          )}
  ...
  return (
    <div data-testid="breadcrumb" className="px-5 py-2.5">
      <div className="flex items-center gap-2 overflow-x-auto pb-1">
        <span className="shrink-0 text-[10px] font-semibold tracking-wider text-surface-500 uppercase select-none">
          Path
        </span>
