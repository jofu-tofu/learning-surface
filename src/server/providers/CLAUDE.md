...
- All system prompts are defined in `server/system-prompt.ts` — `TEACHING_PROMPT` (shared persona + pedagogy), `SYSTEM_PROMPT` (API mode), `CLI_SYSTEM_PROMPT` (CLI mode). If prompt content changes, update that one file.
